...
def generate_launch_description():
    ...
    motor_command_distributor_node = Node(
        package='robot_description', 
        executable='motor_command_node', 
        name='motor_command_distributor_node', 
        output='screen',
        parameters=[
            {'use_sim_time': True} 
        ]
    ...
    test_path_node = Node(
        package='robot_local_localization',
        executable='test_path_node', 
        name='test_path_node',
        output='screen',
        parameters=[
            {'use_sim_time': True},
            {'ekf_odom_topic': '/ekf/odom'}, 
            {'prediction_odom_topic': '/wheel_odom/odom'},
            {'ground_truth_odom_topic': '/odom'}, 
            {'ekf_measurement_topic': '/ekf/measurement'},
            {'side_length': 3.0},
            {'linear_speed': 0.2},
        ]
    )
    ...
    measurement_node = Node(
        ...
        parameters=[
            ...
            {"vo_topic": "/vo/odom"}, 
            {"measurement_topic": "/ekf/measurement"},
            {"odom_frame": "odom"},
    ...
    ekf_node = Node(
        ...
        output='screen',
        parameters=[
            {"prediction_topic": "/wheel_odom/odom"},  
            {"measurement_topic": "/ekf/measurement"}, 
            {"ekf_odom_topic": "/ekf/odom"},          
            {"ekf_rate": 50.0}
        ]
    ...
    return LaunchDescription([
        ...
        motor_command_distributor_node, 
        rtabmap_vo_node,
        prediction_node,
        test_path_node,
        measurement_node,
